...
class WaveView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : View(context, attrs, defStyleAttr) {
     ...
     * 波浪圆圈颜色
     */
    private var mColor = resources.getColor(R.color.blue)
    /**
     * 第一个圆圈的半径(也就是圆形图片的半径)
     */
    private var mImageRadius = 100
    /**
     * 波浪圆之间间距
     */
     ...
     * 最大宽度
     */
    private var mMaxRadius: Int? = 600
    /**
     * 是否正在扩散中
     */
    /**
    ...
    // 扩散圆半径集合
    private val mRadius = ArrayList<Int>()
    ...
    init {
        init()
        val a = context.obtainStyledAttributes(attrs, R.styleable.WaveView, defStyleAttr, 0)
    ...
    public override fun onDraw(canvas: Canvas) {
        ...
        for (i in mAlphas.indices) {
            ...
            if (alpha > 0 && /*mImageRadius + */radius < mMaxRadius!!) {
                ...
                mAlphas[i] = alpha
                mRadius[i] = radius+2
            } else if (alpha < 0 && mImageRadius + radius > this.mMaxRadius!!) {
